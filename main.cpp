...
int getPosJugadores(Jugador *jugadores[], int num, int size){
    for(int i = 0; i < size; i++){
        if(jugadores[i]->getCurrentPos() == num){
            return i;
        }
    }
    return 0;
...
bool isInJuagdores(Jugador *jugadores[], int num, int size){
    bool flag = false;
    for(int i = 0; i < size; i++){
        if(jugadores[i]->getCurrentPos() == num){
            flag = true;
...
void imprimirTablero2(Casilla *tablero[], int size, Jugador *jugadores[], int jugSize){
    for(int i = size-1; i >= 0; i--){
        if((i+1) % (size/10) == 0){
            cout << "\n";
        }
        if(isInJuagdores(jugadores, i, jugSize)){
            int pos = getPosJugadores(jugadores, i, jugSize);
            cout << "J";
            cout << (pos < 10 ? "0" : "");
            cout << pos+1 << "     ";
        } else{
...
void cambiarPos(Jugador *jugadores[], int num, int lastPos, int size){
    for(int i = 0; i < size; i++){
        if(jugadores[i]->getCurrentPos() == jugadores[num]->getCurrentPos() && i != num && jugadores[i]->getCurrentPos() != 0){
            cout << "Jugador " << (i+1) << " te han alcanzado..tu nueva posición es: " << lastPos << "\n";
            jugadores[i]->setCurrentPos(lastPos);
        }
    }
}
...
int main(){
    ...
    while(!terminar && partidas < (t/2)){
        ...
        for(int i = 0; i < numJugadores; i++){
            ...
            int lastPos = jugadores[i]->getLastPos();
            cout << "\n--Turno jugador " << (i + 1) << "\n";
            cout << "--Posición actual: " << (jugadores[i]->getCurrentPos()+1) << "\n";
            cout << "--Número tirado: " << num << "\n";
            pos = jugadores[i]->getCurrentPos() + num;//nueva posición
            if(pos < t-1){
                //posición según serpientes y escaleras
                pos = (jugadores[i]->getCurrentPos() + getNewPos(pos, tablero, i + 1));
                cout << "Nueva posición1: " << pos << "\n";
                jugadores[i] += pos;
                cout << "Nueva posición: " << (jugadores[i]->getCurrentPos() + 1) << "\n";
                ...
                jugadores[i]->setCurrentPos(t - 1);
            }
            if(jugadores[i]->getCurrentPos() >= (t-1)){
