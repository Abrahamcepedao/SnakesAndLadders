...
bool isIn(vector<int> vect, int num){
    bool flag = false;
...
void crearSerpientes(vector<int> &serp,int s, int n){
    int num;
    srand(time(NULL));
    for(int i = 0; i < s; i++){
        num = rand() % n + 5;
...
void crearEscaleras(vector<int> &esc, vector<int> serp, int e, int s, int n){
    int num;
    srand(time(NULL));
    for(int i = 0; i < e; i++){
        num = rand() % n + 5;
        while(isIn(serp, num) || isIn(esc, num)){
            num = rand() % n + 5;
        }
        esc.push_back(num);
    }
...
bool hasTwo(vector<int> vect, int num){
    int count =0;
    for(int i = 0; i < vect.size(); i++){
        if(vect[i] == num){
            count++;
...
void crearTablero(Casilla *tablero[], vector<int> serp, vector<int> esc, int n){
    vector<int> serpientes;
    vector<int> escaleras;
    for(int i = 0; i < n; i++){
        if(isIn(serp, i)){
            //crear serpiente
            srand(time(NULL));
            int max = i - 2;
            int num = rand() % max;
            while(hasTwo(serpientes,num)){
                num = rand() % max;
            }
            serpientes.push_back(num);
            ...
            num = num > (n-1) ? (n-1) : num;
            while(hasTwo(escaleras,num)){
                num = rand() % max;
            }
            escaleras.push_back(num);
...
int getPos(vector<int> jugadores, int num){
...
void imprimirTablero2(Casilla *tablero[], int size, vector<int> jugadores){
    for(int i = size-1; i >= 0; i--){
        if((i+1) % (size/10) == 0){
            cout << "\n";
        }
        if(isIn(jugadores, i)){
            int pos = getPos(jugadores, i);
            cout << "J";
            cout << (pos < 10 ? "0" : "");
            cout << pos+1 << "     ";
        } else{
...
void cambiarPos(vector<int> &jugadores, int num, int lastPos){
    for(int i = 0; i < jugadores.size(); i++){
...
int main(){
    ...
    //crear tablero
    Casilla *tablero[t];
    crearTablero(tablero, serpientes, escaleras, t);
    ...
    while(!terminar && partidas < (t/2)){
        ...
        for(int i = 0; i < numJugadores; i++){
            ...
            if(pos < t-1){
                ...
                cambiarPos(jugadores, i, lastPos);
                
                imprimirTablero2(tablero, t, jugadores);   
            }
            if(jugadores[i] >= (t-1)){
